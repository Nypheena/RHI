...
- Fixed OptiScaler uninstall deleting the game's original DLSS files (`nvngx_dlss.dll`, `nvngx_dlssd.dll`, `nvngx_dlssg.dll`) after correctly restoring them. The DLSS DLLs were being processed twice — once by the dedicated restore steps (which correctly restored them from backup) and again by the generic file cleanup loop (which deleted the just-restored files). The generic loop now skips all four DLSS DLL names since they are handled by their own dedicated steps.
